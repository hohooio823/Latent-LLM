...
def get_correct_answers(data):
    correct_data = data['correct_index']
    label = data['label']
    correct_data_id = label.index(correct_data)
    correct_answer = data['answer'][correct_data_id]
    return f"{data['question']}{correct_answer}"
    

def construct_few_shot(data, num_shots=5, seed=None):
    import numpy as np 
    import random 
    ...
    sampled_data = data[:num_shots]
    other_data = data[num_shots:]

    sample_data_cleaned = [get_correct_answers(data) for data in sampled_data]

    return sample_data_cleaned, other_data