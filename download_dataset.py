...
class DatasetDownloader:
    ...
    def __init__(self, cache_dir="data_owt", tokenizer_name="gpt2", data_files_limit=None):
        ...
        Args:
            ...
            data_files_limit (int): Max number of data files (chunks) to download.
    ...
    def download_openwebtext(self, split="train", num_samples=None):
        ...
        if split == "train":
            # Try different OpenWebText sources
            datasets_to_try = [
                ("olivercareyncl/openwebtext", "train"),
                ("allenai/c4", "train"),
                ("wikitext", "wikitext-2-raw-v1"),
                ("pile", "train")
            ]
        else:
            # For validation/test, use smaller datasets
            datasets_to_try = [
                ("wikitext", "wikitext-2-raw-v1"),
                ("allenai/c4", "validation"),
                ("olivercareyncl/openwebtext", "validation")
            ]

        for dataset_name, dataset_split in datasets_to_try:
            try:
                logger.info(f"Trying dataset: {dataset_name} ({dataset_split})")

                # --- START OF MODIFICATION ---
                # If we have a limit on data files and targeting openwebtext train,
                # download only the first N parquet files
                if dataset_name == "olivercareyncl/openwebtext" and self.data_files_limit is not None and split == "train":
                    logger.info(f"Limiting download to {self.data_files_limit} data file(s).")
                    # Construct list of file paths: train-00000-of-00019.parquet, etc.
                    files_to_load = [f"split/chunk_{i}.json" for i in range(1, self.data_files_limit + 1)]
                    dataset = load_dataset(
                        dataset_name,
                        data_files=files_to_load,
                        split=split
                    )
                # --- END OF MODIFICATION ---

                elif dataset_name in ["wikitext", "pile"]:
                    # These datasets need config name
        ...
        # If all datasets fail, create a small synthetic dataset
        logger.error("All dataset loading attempts failed. Creating synthetic dataset for testing...")
        return Dataset.from_dict({"text": [
            "This is a synthetic document for testing.",
            "Another short synthetic text for verification."
        ]})
    ...
    def preprocess_dataset(self, dataset, split="train", num_workers=4, max_length=1024):
        ...
        def concatenate_and_wrap(texts, tokenizer):
            """Concatenate texts and wrap to max_length with EOS tokens."""
            all_tokens = []
            current_tokens = []

            for text in texts:
                # Tokenize the text using the instance tokenizer
        ...
        def process_batch(examples, tokenizer, max_length):
            ...
            tokenized_segments = concatenate_and_wrap(texts, tokenizer)
        ...
        if len(dataset) > 10000:
            with ProcessPoolExecutor(max_workers=num_workers) as executor:
                ...
                for i in range(0, len(dataset), batch_size):
                    ...
                    futures.append(executor.submit(process_batch, batch, self.tokenizer, max_length))

                results = []
                for future in tqdm(futures, desc="Processing batches"):
                    results.extend(future.result())
        else:
            results = process_batch(list(dataset), self.tokenizer, max_length)
...
def main():
    ...
    parser.add_argument("--data-files-limit", type=int, default=None,
                        help="Limit the number of data files (chunks) to download for testing (e.g., 1 for one chunk)")
