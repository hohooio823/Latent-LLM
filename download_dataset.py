...
class DatasetDownloader:
    ...
    def download_openwebtext(self, split="train", num_samples=None):
        ...
        # Map user-friendly split names to actual dataset split names
        split_map = {
            "train": "train",
            "val": "validation",
        ...
        datasets_to_try = [
            ...
            ("allenai/c4", hf_split),
            ("wikitext", "wikitext-2-raw-v1"), # Wikitext has fixed split names
            ("pile", hf_split)
        ] if split == "train" else [
        ...
        for dataset_name, dataset_split in datasets_to_try:
            try:
                # For wikitext, we need to use the user-provided split name
                current_split = hf_split if dataset_name != "wikitext" else split
                if current_split == "val": current_split = "validation" # Ensure mapping for wikitext

                logger.info(f"Trying dataset: {dataset_name} (config: {dataset_split}, split: {current_split})")
                
                # FIXED: The --data-files-limit now works for ANY split.
                ...
                elif dataset_name == "allenai/c4": # C4 needs a config name
